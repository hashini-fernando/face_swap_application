...
def angular_diff(a, b):
    """Compute the shortest angular difference between two angles (0-180°)"""
    diff = abs(a - b) % 360
    return diff if diff <= 180 else 360 - diff

def get_face_angle_compatibility(src_pose, dst_pose):
    """Check if face angles are compatible for swapping using shortest angular difference"""
    angle_threshold = 45  # threshold for warnings
...
def compute_face_detection_rate(app, face_image, target_image):
    ...
    detection_rate = (face_detected + target_detected) / 2 * 100
    return detection_rate

def compute_pose_metrics(face_obj, target_obj):
    """Return yaw/pitch/roll for source and target, and angle compatibility"""
...
def compute_identity_similarity(swapper, source_face, target_face):
    """Compute similarity between original and swapped face embeddings"""
    try:
        # Get embeddings (feature vectors) from swapper
        src_feat = swapper.get_feat(to_bgr(source_face))
        swapped_feat = swapper.get_feat(to_bgr(target_face))
        
        # Cosine similarity
        cos_sim = np.dot(src_feat, swapped_feat) / (np.linalg.norm(src_feat) * np.linalg.norm(swapped_feat))
        return float(cos_sim) * 100  # Percentage
    except Exception:
        return None
...
with col3:
    ...
    if st.session_state.swapped_image is not None:
        ...
        # ---------------------------
        st.markdown("## 📊 Swapped Face Metrics")
        ...
        face_faces = app.get(face_cv2)
        target_faces = app.get(target_cv2)
        swapped_faces = app.get(swapped_cv2)

        # 1️⃣ Face Detection Rate
        detection_rate = compute_face_detection_rate(app, st.session_state.face_image, st.session_state.swapped_image)
        st.metric(label="✅ Face Detection Rate (%)", value=f"{detection_rate:.1f}")

        # 2️⃣ Angle Compatibility
        if face_faces and target_faces:
            pose_metrics = compute_pose_metrics(face_faces[0], target_faces[0])
            st.metric(label="🔄 Angle Compatibility Score (%)", value=f"{pose_metrics['angle_compatibility_score']:.1f}")
            if pose_metrics['warnings']:
                for w in pose_metrics['warnings']:
                    st.warning(w)
        else:
            pose_metrics = None

        # 3️⃣ Identity Preservation
        if face_faces and swapped_faces:
            # Crop the first detected face from source and swapped
            src_face_crop = to_pil(face_cv2[
                int(face_faces[0].bbox[1]):int(face_faces[0].bbox[3]),
                int(face_faces[0].bbox[0]):int(face_faces[0].bbox[2])
            ])
            
            swapped_face_crop = to_pil(swapped_cv2[
                int(swapped_faces[0].bbox[1]):int(swapped_faces[0].bbox[3]),
                int(swapped_faces[0].bbox[0]):int(swapped_faces[0].bbox[2])
            ])
            
            identity_score = compute_identity_similarity(swapper, src_face_crop, swapped_face_crop)
            if identity_score is not None:
                st.metric(label="🧬 Identity Preservation (%)", value=f"{identity_score:.1f}")
        else:
            st.warning("⚠️ Could not compute identity preservation: face not detected in source or swapped image.")
